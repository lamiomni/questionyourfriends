...
namespace QuestionYourFriends.Controllers
{
    public class MyQuestionsController : BaseController
    {
        ...
        public string GetMyQuestion(dynamic res)
        {
            ...
            // Stocker dans result la liste des questions dont le récepteur est l'utilisateur
            foreach (QuestionYourFriendsDataAccess.Question a in BusinessManagement.Question.GetListOfReceiver(user.id))
            {
                ...
                result += a.text;
                result += " ";
                // on va chercher le nom et prénom de l'envoyeur en comparant les fids de la base de données et de Session["friend"]

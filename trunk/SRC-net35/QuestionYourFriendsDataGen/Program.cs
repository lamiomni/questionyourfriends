...
namespace QuestionYourFriendsDataGen
{
    ...
    public static class Program
    {
        ...
        private static void CleanDb()
        {
            ...
            {
                _qyfe.DeleteObject(transaction);
                i++;
            }
            Console.Write(@".");
            _qyfe.SaveChanges();
        ...
        private static void AddData()
        {
            ...
            i = 0;
            var schizoQuestions = Enumerable.ToList(Queryable.Where(qs, question => question.Receiver.id == question.Owner.id));
            foreach (var question in schizoQuestions)
            {
                _qyfe.DeleteObject(question);
                i++;
            }
            ...
            int qyfid = BusinessManagement.User.Get((long)0).id;
            qs = _qyfe.Questions;
            Console.Write(@"      - Anti-questions-to-Qyf check.");
            i = 0;
            var toQyfQuestions = Enumerable.ToList(Queryable.Where(qs, question => question.Receiver.id == qyfid));
            foreach (var question in toQyfQuestions)
            {
                _qyfe.DeleteObject(question);
                i++;
            }
            ...
            // Add transactions
            User jr = BusinessManagement.User.Get(FidJr);
            User tony = BusinessManagement.User.Get(FidTony);
            User antony = BusinessManagement.User.Get(FidAntony);
            User victor = BusinessManagement.User.Get(FidVictor);
            Console.Write(@"      - Transacs");
            var transacs = _session.List<Transac>(nbTransac)
                .First(1)
                    .Impose(u => u.User, tony)
                .Next(1)
                    .Impose(u => u.User, jr)
                .Next(1)
                    .Impose(u => u.User, antony)
                .Next(1)
                    .Impose(u => u.User, victor)
                .All()
                    .Get();
            Console.Write(@".");
            i = 0;

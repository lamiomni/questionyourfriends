...
namespace AutoPoco
{
    public static class StandardExtensions
    {
        ...
        public static IEngineConfigurationTypeBuilder<TPoco> Value<TPoco, TMember>(
            this IEngineConfigurationTypeMemberBuilder<TPoco, TMember> memberConfig, TMember value)
        {
        ...
        public static IEngineConfigurationTypeBuilder<TPoco> FromParent<TPoco, TMember>(
            this IEngineConfigurationTypeMemberBuilder<TPoco, TMember> memberConfig)
        {
            ...
            where TCollection : IEnumerable
        {
            ...
            if (collectionContentType == null)
            {
                throw new ArgumentException(
                    "Unable to find collection type, only collections of type IEnumerable<T> are supported",
                    "TCollection");
            }

            // So this will give us AutoSource<Y>
            Type autoSourceType = typeof (AutoSource<>).MakeGenericType(collectionContentType);

            //// And this will give us FlexibleEnumerableSource<AutoSource<Y>, X<Y>, Y>
            Type enumerableSourceType = typeof (FlexibleEnumerableSource<,,>).MakeGenericType(
            ...
            MethodInfo method = memberConfig.GetType().GetMethod("Use",
                                                                 BindingFlags.Public |
                                                                 BindingFlags.Instance, null,
                                                                 new[] {typeof (Object[])}, null);

            MethodInfo genericMethod = method.MakeGenericMethod(new[] {enumerableSourceType});
            return
                (IEngineConfigurationTypeBuilder<TPoco>)
                genericMethod.Invoke(memberConfig, new object[] {new object[] {min, max}});
        }
